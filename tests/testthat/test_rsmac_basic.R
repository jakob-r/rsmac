...
test_that("very basic rsmac works", {
  ...
  params = c("x1 real [-5.12,5.12] [3]", "x2 real [-5.12,5.12]  [3]")
  res = rsmac(fun, scenario = scenario, params = params, cleanup = FALSE, id.smac.run = "verybasic")
  expect_class(res, "OptPath")
  expect_equal(getOptPathLength(res), 20)
  expect_equal(nrow(unique(getOptPathX(res))), 20)
...
test_that("rsmac wit as.pcs works with more param spaces", {
  on.exit({
    unlink("~/bin/smac/rsmac_autopcs2", recursive = TRUE)
    unlink("~/bin/smac/smac-output/rsmac-scenario-autopcs2", recursive = TRUE)
  })
  fun = function(x) {
